...
- [ckeditor assets precompile 문제](https://github.com/galetahub/ckeditor/issues/663)
- 위 문제는 config/environments/production.rb 에 `config.assets.js_compressor = :uglifier`를 주석처리하여 임시로 해결. 시간이되면 ckeditor 커밋 or 누군가 이슈를 해결해주면 다시 js comprss 하겠음.
